...
- Inspect-edit-review lifecycle for existing files, including isolated model review of the before/after diff and focused full-file repair when review fails.
...
-> model-extracted task contract validated by LocalAgent
...
- [local_agent/task_contract.py](local_agent/task_contract.py): model contract normalization, fallback task obligations, evidence ledger construction, and contract-based finish checks.
...
The task contract layer asks the model to extract structured obligations such as workspace changes, file deletion, discovery, source inspection/reporting, semantic edit review, local execution, test evidence, visible output, assistant-response duties, repeated successful executions, and ordering constraints. For existing-file updates, the harness forces source inspection before mutation and an isolated before/after source review afterward. If an inspected target still has an unmet mutation obligation, the next planning call is constrained to `write_file` or `replace_in_file`. It validates and normalizes the contract, builds an evidence ledger from actual observations, and rejects premature `finish`/`answer` actions when required evidence is missing. Set `contract_mode` to `fallback` to disable model contract extraction and use the deterministic fallback path.

Repair planning is also validated outside the model. The harness rejects duplicate failed shell commands before rerunning them, blocks repeated identical rewrites, keeps command comparisons normalized across common Python invocation forms, runs Python tests according to source structure, can repair simple expected-literal mistakes during test-only edits, and stops when the model cannot produce a corrective action after a failed verification. Final replies include only contract-relevant source and command evidence, so stale pre-edit runs are not repeated unless the user explicitly requested before/after execution.
